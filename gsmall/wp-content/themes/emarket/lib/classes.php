...
if ( !class_exists('Emarket_Widget') ):

abstract class Emarket_Widget extends WP_Widget{
	...
	protected function init(){
		...
		$this->base_tpl_path = apply_filters('emarket_widget_template_base', $this->base_path.'/widgets/'.$this->id_base.'/tmpl');
		$this->override_tpl_path = apply_filters('emarket_widget_override_base', get_template_directory().'/widgets/'.$this->id_base);
	}
	...
	public function emarket_trim_words( $text, $num_words = 30, $more = null ) {
...
if ( (is_string($opts['multiple']) && strtolower($opts['multiple'])=='multiple') || (is_bool($opts['multiple']) &&
$opts['multiple']) ){
...
if ( (is_string($opts['disabled']) && strtolower($opts['disabled'])=='disabled') || is_bool($opts['disabled']) &&
$opts['disabled'] ){
...
if ( (is_string($opts['required']) && strtolower($opts['required'])=='required') || (is_bool($opts['required']) &&
$opts['required']) ){
...
$select_html = '<select ' . $select_attributes . '>';
    ...
    $select_html .= '<optgroup label="'. esc_attr__('Override by Theme', 'emarket') .'">';
        foreach ($templates['ov'] as $name => $label) {
        $select_html .= '<option value="' . $name . '"';
				if ($name == $field_value || (is_array($field_value)&&in_array($name, $field_value))){ $select_html .= '
            selected="selected"';}
    ...
    $select_html .= '<optgroup label="'. esc_attr__('Default Template', 'emarket') .'">';
        foreach ($templates['df'] as $name => $label) {
        $select_html .= '<option value="' . $name . '"';
				if ($name == $field_value || (is_array($field_value)&&in_array($name, $field_value))){ $select_html .= '
            selected="selected"'; }
				if (array_key_exists(' ov', $templates) && array_key_exists($name, $templates['ov'])){ $select_html
            .=' disabled="disabled"' ; } $select_html .='>' .$label.'</option>';
            };
            $select_html .= '</optgroup>';
    }
    $select_html .= '</select>';
...
$allow_select_all = '<option value="0">'. esc_html_e( 'All Categories', 'emarket' ) .'</option>';
}
}
...
$select_html = '<select ' . $select_attributes . '>';
    ...
    $select_html .= '<option
        value="' . esc_attr( $cat->cat_ID ) . '"';
			if ($cat->cat_ID == $field_value || (is_array($field_value)&&in_array($cat->cat_ID, $field_value))){ $select_html .= '
        selected="selected"';}
...
$select_html = '<select ' . $select_attributes . '>';
    ...
    $select_html .= '<option
        value="' . esc_attr( $cat->term_id ) . '"';
			if ($cat->term_id == $field_value || (is_array($field_value)&&in_array($cat->term_id, $field_value))){ $select_html .= '
        selected="selected"';}
			$select_html .=  '>'.$cat->name.'</option>';
    }
    $select_html .= '</select>';
...
?>
<p>
    <label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title', 'emarket')?></label>
    <br />
...
<p>
    <label
        for="<?php echo esc_attr( $this->get_field_id('widget_template') ); ?>"><?php esc_html_e("Template", 'emarket') ?></label>
    <br />
