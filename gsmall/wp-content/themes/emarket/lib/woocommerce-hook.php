...
function emarket_product_thumbnail( $size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0  ) {
	...
	if ( has_post_thumbnail( $post->ID ) ){
		if( $attachment_image ){
			...
			$html .= (get_the_post_thumbnail( $post->ID, $size, array( 'alt' => $post->post_title ) )) ? get_the_post_thumbnail( $post->ID, $size, array( 'alt' => $post->post_title ) ): '<img src="'.get_template_directory_uri().'/assets/img/placeholder/'.$size.'.png" alt="">';
...
function emarket_before_shop_loop_wrapper_end()
{
    ...
    echo '</div>';
    if (is_active_sidebar('above-product') && emarket_sidebar_product() == 'full' && $product_filter_style != 'accordion' ) {
        ?>
...
function emarket_woommerce_view_mode_wrap () {
	...
	$html = '<div class="view-mode-wrap pull-left clearfix">
				<div class="view-mode">
						<a href="javascript:void(0)" class="grid-view '. esc_attr( $class_grid ) .'" title="'. esc_attr__('Grid view', 'emarket').'"><span>'. esc_html__('Grid view', 'emarket').'</span></a>
						<a href="javascript:void(0)" class="list-view2 '. esc_attr( $class_list ) .'" title="'. esc_attr__('List view2', 'emarket') .'"><span>'.esc_html__('List view2', 'emarket').'</span></a>
				</div>	
			</div>';
	echo $html;
}
...
<div class="product_meta">
    ...
    <span class="sku_wrapper"><?php esc_html_e( 'SKU:', 'emarket' ); ?> <span class="sku"
            itemprop="sku"><?php echo ( $sku = $product->get_sku() ) ? $sku : esc_html__( 'N/A', 'emarket' ); ?></span></span>
...
<div class="item-brand <?php echo emarket_options()->getCpanelValue( 'product_brand' ) ? 'brand-image' : '';?>">
    ...
    <a href="<?php echo get_term_link( $term->term_id, 'product_brand' ); ?>"><img
            src="<?php echo wp_get_attachment_thumb_url( $thumbnail_id ); ?>" alt=""
