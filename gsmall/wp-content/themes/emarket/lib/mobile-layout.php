...
function emarket_header_check(){ 	
	...
	*/
	...
	if( emarket_mobile_check() ):
		...
		if( defined( 'ELEMENTOR_VERSION' ) && \Elementor\Plugin::$instance->db->is_built_with_elementor( $header_mobile_elementor ) ){
...
function emarket_footer_check(){
	...
	if( emarket_mobile_check() && $mobile_footer != '' ):
		...
		if( defined( 'ELEMENTOR_VERSION' ) && \Elementor\Plugin::$instance->db->is_built_with_elementor( $footer_mobile_elementor ) ){
...
function sw_custom_template_include( $theme_template_id ){
	if( ( is_post_type_archive( 'product' ) || is_singular( 'product' ) || is_tax( 'product_cat' ) || is_tax( 'product_brand' ) ) && emarket_mobile_check() ){
		$theme_template_id = 0;
	}
