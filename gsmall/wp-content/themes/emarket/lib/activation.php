...
<div class="sw-activation-form">
    <div class="activation-form-inner">
        <h2>
            ...
            <a href="<?php echo esc_url( 'http://wpthemego.com/document/how-to-get-purchase-code-for-items-from-envato/' ); ?>"
                target="_blank" class="sw-activation-help"
                title="<?php echo esc_attr__( 'Need help? Please follow this url', 'emarket' ); ?>"><?php echo esc_html__( 'Help', 'emarket' ); ?></a>
        </h2>
        <?php settings_errors(); ?>
                ...
                <ul>
                    <li>
                        <label for="sw_purchase_code" class="clearfix">
                            <input type="text" id="sw_purchase_code"
                                placeholder="<?php echo esc_attr__( 'Enter Your Purchase Code', 'emarket' ); ?>"
                                name="sw_purchase_code"
                                value="<?php echo esc_attr( get_option( 'sw_purchase_code' ) ); ?>" />
                        </label>
                    </li>
                </ul>
            </div>
	...
	if( sw_verify_purchase_code_result( $input ) ) {
		$new_input = $input; 
		 ...
		 add_settings_error(
			...
			esc_html__( 'Your Purchase Code are invalid, please fill correct code to action theme', 'emarket' ),
			'error'
		);
	}
	return  $new_input;
...
// if( !get_option( 'sw_purchase_code' ) ) {
	// function sw_admin_notice_validate_purchase_code() {
		...
		// $message = esc_html__( 'Your theme is not active now, please fill your purchase code to active your theme!', 'emarket' );
