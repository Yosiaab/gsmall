				...
				if( $p->product_type == "simple" ) {
					...
					foreach ( $childposts as $child ) {
						if( !in_array( $child->ID, $bundles ) ) {
							...
							$html .= '<li class="swpb-product"><a href="#" product_id="'. $variation->get_id() .'" product_type="simple" title="'. esc_attr( trim( $title ) ) .'">#'. $variation->get_id() .' - '. esc_html( $product->post_title ) .' '. esc_html( trim( $title ) ) .'</a></li>';
						}						
					}
				}	
			}				
