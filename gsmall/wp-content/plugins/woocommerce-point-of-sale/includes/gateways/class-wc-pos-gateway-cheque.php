...
class WC_POS_Gateway_Cheque extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_filter( 'wc_pos_params', array( $this, 'params' ) );
	}
	...
	public function params( $params ) {
		...
		$params['cheque_instructions'] = $settings['instructions'];
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			...
			$order->update_status( apply_filters( 'woocommerce_cheque_process_payment_order_status', 'on-hold', $order ), _x( 'Awaiting check payment', 'Check payment method', 'woocommerce-point-of-sale' ) );
		} else {
