...
class WC_POS_Report_Sales_By_Register extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s gross sales from this register', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s net sales from this register', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->net_sales ) . '</strong>' ),
	...
	public function current_filters() {
		...
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'register_ids' ) ) . '">' . esc_html__( 'Reset', 'woocommerce-point-of-sale' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php esc_html_e( 'Register Search', 'woocommerce-point-of-sale' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<select class="wc-product-search" style="width:203px;" name="register_ids[]" data-placeholder="<?php esc_attr_e( 'Search for a register&hellip;', 'woocommerce-point-of-sale' ); ?>" data-action="wc_pos_json_search_registers"></select>
		...
		<h4 class="section_title"><span><?php esc_html_e( 'Top Registers', 'woocommerce-point-of-sale' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				...
				if ( $top_sellers ) {
					foreach ( $top_sellers as $register ) {
						$reg      = wc_pos_get_register( absint( $register->register_id ) );
					...
					echo '<tr><td colspan="3">' . esc_html__( 'No registers found in range', 'woocommerce-point-of-sale' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
	...
	public function get_main_chart() {
		...
		if ( ! $this->register_ids ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php esc_html_e( '&larr; Choose a register to view stats', 'woocommerce-point-of-sale' ); ?></p>
			</div>
			<?php
		} else {
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						...
						{
							label: "<?php echo wp_kses_post( __( 'Coupon amount', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo wp_kses_post( __( 'Refund amount', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo wp_kses_post( __( 'Shipping amount', 'woocommerce-point-of-sale' ) ); ?>",
