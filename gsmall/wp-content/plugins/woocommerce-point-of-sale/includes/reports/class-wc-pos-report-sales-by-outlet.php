 ...
 *
 * @package WooCommerce_Point_Of_Sale/Classes/Reports
 */
...
class WC_POS_Report_Sales_By_Outlet extends WC_Admin_Report {
	...
	public function __construct() {
		...
		if ( $this->outlet_ids && is_array( $this->outlet_ids ) ) {
			foreach ( $this->outlet_ids as $outlet_id ) {
				$regs = wc_pos_get_registers_by_outlet( $outlet_id );
				if ( $regs ) {
	...
	public function get_chart_legend() {
		...
		switch ( $this->chart_groupby ) {
			case 'day':
				...
				$average_sales_title = sprintf( __( '%s average daily sales', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->average_sales ) . '</strong>' );
				break;
			case 'month':
			default:
				/* translators: %s average sales */
				$average_sales_title = sprintf( __( '%s average monthly sales', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->average_sales ) . '</strong>' );
				break;
		}

		$legend[] = array(
			/* translators: %s total sales */
			'title'            => sprintf( __( '%s gross sales from this outlet', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the order totals after any refunds and including shipping and taxes.', 'woocommerce-point-of-sale' ),
			'color'            => $this->chart_colours['sales_amount'],
			'highlight_series' => 6,
		);
		$legend[] = array(
			/* translators: %s net sales */
			'title'            => sprintf( __( '%s net sales from this outlet', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->net_sales ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the order totals after any refunds and excluding shipping and taxes.', 'woocommerce-point-of-sale' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s sales from this register for saved orders', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->saved_sales ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the saved order totals after any refunds and including shipping and taxes.', 'woocommerce-point-of-sale' ),
			'color'            => $this->chart_colours['saved_sales_amount'],
			'highlight_series' => 8,
		);
		$legend[] = array(
			/* translators: %s number of orders */
			'title'            => sprintf( __( '%s orders placed', 'woocommerce-point-of-sale' ), '<strong>' . ( $data->total_order_refunds + $data->total_orders !== $data->total_orders ? '<del>' . ( $data->total_order_refunds + $data->total_orders ) . '</del> ' : '' ) . $data->total_orders . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s items purchased', 'woocommerce-point-of-sale' ), '<strong>' . ( $data->total_item_refunds + $data->total_partial_refunds > 0 ? '<del>' . ( $data->total_item_refunds + $data->total_partial_refunds + $data->total_items ) . '</del> ' : '' ) . $data->total_items . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s charged for shipping', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->total_shipping ) . '</strong>' ),
			'color'            => $this->chart_colours['shipping_amount'],
			'highlight_series' => 5,
		);
		$legend[] = array(
			/* translators: %s total refunds */
			'title'            => sprintf( __( '%s in refunds', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->total_refunds ) . '</strong>' ),
			'color'            => $this->chart_colours['refund_amount'],
			'highlight_series' => 4,
		);
		$legend[] = array(
			/* translators: %s total coupons */
			'title'            => sprintf( __( '%s worth of coupons used', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->total_coupons ) . '</strong>' ),
	...
	public function output_report() {
		...
		$ranges = array(
			'year'       => __( 'Year', 'woocommerce-point-of-sale' ),
			'last_month' => __( 'Last Month', 'woocommerce-point-of-sale' ),
			'month'      => __( 'This Month', 'woocommerce-point-of-sale' ),
			'7day'       => __( 'Last 7 Days', 'woocommerce-point-of-sale' ),
		);
	...
	public function get_chart_widgets() {
		...
		if ( ! empty( $this->register_ids ) ) {
			$widgets[] = array(
				'title'    => __( 'Showing reports for:', 'woocommerce-point-of-sale' ),
	...
	public function current_filters() {
		...
		foreach ( $this->outlet_ids as $outlet_id ) {
			$outlet                    = wc_pos_get_outlet( absint( $outlet_id ) );
			$this->outlet_ids_titles[] = $outlet ? $outlet->get_name() : '#' . $outlet_id;
		}

		echo '<p><strong>' . esc_html( implode( ', ', $this->outlet_ids_titles ) ) . '</strong></p>';
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'outlet_ids' ) ) . '">' . esc_html__( 'Reset', 'woocommerce-point-of-sale' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php esc_html_e( 'Outlet Search', 'woocommerce-point-of-sale' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<select class="wc-product-search" style="width:203px;" name="outlet_ids[]" data-placeholder="<?php esc_attr_e( 'Search for a outlet&hellip;', 'woocommerce-point-of-sale' ); ?>" data-action="wc_pos_json_search_outlet" ></select>
					<input type="submit" class="submit button" value="<?php esc_attr_e( 'Show', 'woocommerce-point-of-sale' ); ?>" />
		...
		<h4 class="section_title"><span><?php esc_html_e( 'Top Outlets', 'woocommerce-point-of-sale' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				global $wpdb;
				...
				$top_sellers = $wpdb->get_results(
					...
					LEFT JOIN {$wpdb->postmeta} pm_reg ON (orders.ID = pm_reg.post_id AND pm_reg.meta_key = 'wc_pos_id_register')
					LEFT JOIN {$wpdb->postmeta} pm_type ON (orders.ID = pm_type.post_id AND pm_type.meta_key = 'wc_pos_order_type')
				...
				if ( $top_sellers ) {
					foreach ( $top_sellers as $outlet ) {
						$get_outlet = wc_pos_get_outlet( absint( $outlet->outlet_id ) );
					...
					echo '<tr><td colspan="3">' . esc_html__( 'No outlets found in range', 'woocommerce-point-of-sale' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
		<h4 class="section_title"><span><?php esc_html_e( 'Top Earners', 'woocommerce-point-of-sale' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
	...
	public function event_sales_sparkline( $id = '', $days = 7, $type = 'sales' ) {
		if ( $id ) {
			$ids  = array();
			$regs = wc_pos_get_registers_by_outlet( $id );
			if ( $regs ) {
				$ids = $regs;
			}
		...
		if ( 'sales' === $type ) {
			...
			$tooltip = sprintf( __( 'Sold %1$s worth in the last %2$d days', 'woocommerce-point-of-sale' ), strip_tags( wc_price( $total ) ), $days );
		} else {
			/* translators: $1$s number of orders %2$d last number of days */
			$tooltip = sprintf( _n( '%1$d order placed in the last %2$d days', '%1$d orders placed in the last %2$d days', $total, 'woocommerce-point-of-sale' ), $total, $days );
		}
	...
	public function get_export_button() {
		...
		<a
			...
			class="export_csv"
			data-export="chart"
			data-xaxes="<?php esc_attr_e( 'Date', 'woocommerce-point-of-sale' ); ?>"
			data-groupby="<?php echo esc_attr( $this->chart_groupby ); ?>"
		>
			<?php esc_html_e( 'Export CSV', 'woocommerce-point-of-sale' ); ?>
		</a>
		<?php
	}
	...
	public function get_main_chart() {
		...
		if ( ! $this->register_ids ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php esc_html_e( '&larr; Choose outlet to view stats', 'woocommerce-point-of-sale' ); ?></p>
			</div>
			<?php
		} else {
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Number of items sold', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Number of orders', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Average sales amount', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Coupon amount', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Refund amount', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Shipping amount', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Gross Sales amount', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Net Sales amount', 'woocommerce-point-of-sale' ) ); ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Sales amount for saved orders', 'woocommerce-point-of-sale' ) ); ?>",
