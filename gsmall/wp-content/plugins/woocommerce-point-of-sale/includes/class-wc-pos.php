...
class WC_POS {
	...
	public function check_wc_rest_api() {
		try {
			...
			$response    = $client->head( $this->wc_api_url() );
		...
		if ( ! in_array( $status_code, array( 200, 401 ) ) ) {
	...
	public function order_actions_reprint_receipts( $actions, $the_order ) {
		...
		$register_id   = get_post_meta( $the_order->get_id(), 'wc_pos_id_register', true );
	...
	public function add_prefix_suffix_order_number( $order_id, $order ) {
		...
		$register_id = absint( get_post_meta( $order->get_id(), 'wc_pos_id_register', true ) );
