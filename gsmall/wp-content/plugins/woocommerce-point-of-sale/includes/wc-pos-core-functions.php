...
function wc_pos_term_relationships() {
	 $relationships = array();
	$hierarchy      = array();
	$parents        = array();
	...
	$relationships[0] = wc_pos_get_non_cat_products();
...
function wc_pos_tax_enabled() {
	if ( 'enabled' === get_option( 'woocommerce_pos_tax_calculation', 'enabled' ) && wc_tax_enabled() ) {
		return true;
	}
