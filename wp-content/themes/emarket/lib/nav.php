						...
						?><p class="field-<?php echo esc_attr( $field ); ?> description description-wide">
								...
								switch( $field_opts['type'] ):
									...
									?>			
											...
											<?php
												echo ( $item->$field != '') ? '<img class="emarket-opts-screenshot" id="emarket-opts-screenshot-'.esc_attr( $item_id ).'" src="'.esc_attr( $item->$field ).'" width = "30" />': '';
...
class Emarket_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if( ($depth === 0) ){
			...
			$class_names .= ( $item -> mega_full == 1 ) ? ' emarket-megamenu-full' : '';
			$class_names .= ' level1';
		}		
		if( $item -> imgupload != '' ){
		...
		if ( preg_match('/(apc|apcs)/i', $advanced) ){
			...
			if ( preg_match( '/apcs/i', $advanced ) && !empty( $page_content ) ){
				...
				if( defined( 'ELEMENTOR_VERSION' ) && \Elementor\Plugin::$instance->db->is_built_with_elementor( $page_content ) ){
	...
	if (!$args['walker']) {
		if ( 'mega' == emarket_options()->getCpanelValue( 'menu_type' ) && $emarket_theme_locates[0] != NULL && in_array( $args['theme_location'], $emarket_theme_locates ) ){
			...
			if( !emarket_mobile_check() ) :
				wp_enqueue_script('emarket_megamenu', get_template_directory_uri() . '/js/megamenu.min.js', array(), null, true);
			endif;
