...
class WC_POS_Admin_Assets {
	...
	public function admin_styles() {
		...
		wp_register_style( 'wc-pos-print', WC_POS()->plugin_url() . '/assets/dist/css/print.min.css', array(), WC_POS_VERSION );
	...
	public function admin_scripts() {
		...
		wp_register_script( 'wc-pos-admin-receipts', WC_POS()->plugin_url() . '/assets/dist/js/admin/receipts.min.js', array( 'jquery', 'wp-codemirror' ), WC_POS_VERSION, true );
		wp_localize_script(
		...
		if ( isset( $_GET['print_pos_receipt'] ) ) {
			...
			// We only need to enqueue Less.js here.
		...
		if ( in_array( $screen_id, array( 'pos_receipt' ), true ) ) {
			wp_enqueue_media();
		}
