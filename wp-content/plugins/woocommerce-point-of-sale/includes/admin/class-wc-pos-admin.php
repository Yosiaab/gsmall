...
class WC_POS_Admin {
				...
				foreach ( $fieldset['fields'] as $key => $field ) :
					...
					<tr>
						...
						<td>
							...
							<?php } elseif ( 'button' === $field['type'] && 'user_card_number_print' === $key ) { ?>
								<div></div>
							<?php } elseif ( 'button' === $field['type'] && 'user_card_number_scan' === $key ) { ?>
								<div></div>
							<?php } elseif ( 'input' === $field['type'] && 'user_card_number' === $key ) { ?>
	...
	public function get_customer_meta_fields() {
		...
		if ( 'yes' === get_option( 'wc_pos_enable_user_card', 'no' ) ) {
			$show_fields['outlet_fields']['fields']['user_card_number'] = array(
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( plugin_basename( WC_POS_PLUGIN_FILE ) === $file ) {
			$row_meta = array(
				'docs'    => '<a href="' . esc_url( apply_filters( 'wc_pos_docs_url', 'http://actualityextensions.com/documentation/woocommerce-point-of-sale/' ) ) . '" title="' . esc_attr( __( 'View Documentation', 'woocommerce-point-of-sale' ) ) . '">' . __( 'Docs', 'woocommerce-point-of-sale' ) . '</a>',
				'support' => '<a href="' . esc_url( apply_filters( 'wc_pos_docs_url', 'http://actualityextensions.com/contact/' ) ) . '" title="' . esc_attr( __( 'Visit Support', 'woocommerce-point-of-sale' ) ) . '">' . __( 'Support', 'woocommerce-point-of-sale' ) . '</a>',
			);
	...
	public static function create_rewrite_rules( $rules ) {
		global $wp_rewrite;
		$newRule = array(
			'^point-of-sale/([^/]+)/([^/]+)/?$' => 'index.php?page=wc-pos-registers&action=view&outlet=$matches[1]&register=$matches[2]',
			'^point-of-sale/sw/?$'              => ltrim( str_replace( get_home_url(), '', WC_POS()->plugin_url() ), '/' ) . '/assets/service-worker.js',
	...
	public static function on_rewrite_rule() {
		...
		add_rewrite_rule( '^point-of-sale/sw/?$', ltrim( str_replace( get_home_url(), '', WC_POS()->plugin_url() ), '/' ) . '/assets/service-worker.js', 'top' );
	}

	public static function flush_rules() {
		$rules = get_option( 'rewrite_rules' );

		if ( ! isset( $rules['^point-of-sale/([^/]+)/([^/]+)/?$'] ) || ! isset( $rules['^point-of-sale/sw/?$'] ) ) {
			global $wp_rewrite;
	...
	public function wc_settings_button_field( $value ) {
		...
		<th scope="row" class="titledesc">
			<label for=""><?php echo esc_html( $value['title'] ); ?> <?php echo esc_html( $tooltip_html ); ?></label>
		</th>
	...
	public function wc_settings_range_slider( $field ) {
		...
		$field['class'] = empty( $field['class'] ) ? 'range_slider' : $field['class'];
		?>
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for=""><?php echo esc_html( $field['title'] ); ?> <?php echo esc_html( $tooltip_html ); ?></label>
			</th>
