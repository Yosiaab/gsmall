 ...
 */
?>
<h2><?php esc_html_e( 'Denomination Options', 'woocommerce-point-of-sale' ); ?></h2>
<div id="denomination_options-description">
	<p><?php esc_html_e( 'The following table defines the denominations that your point of sale system use when handling cash', 'woocommerce-point-of-sale' ); ?></p>
</div>
<table class="wc-pos-register-denomination-options widefat" id="wc-pos-register-denomination-options">
	...
	<tbody>
		<?php foreach ( $denominations as $index => $denomination ) : ?>
			<?php
			// Let's start from 1 because having an index of 0 breaks the checked attribute for some reason.
			$index++;
			?>
