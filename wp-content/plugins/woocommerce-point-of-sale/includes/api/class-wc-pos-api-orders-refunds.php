...
class WC_API_POS_Orders_Refunds extends WC_REST_Order_Refunds_Controller {
	...
	protected function prepare_object_for_database( $request, $creating = false ) {
		...
		if ( ! $order ) {
			return new WP_Error( 'woocommerce_rest_invalid_order_id', __( 'Invalid order ID.', 'woocommerce-point-of-sale' ), 404 );
		}

		if ( 0 > $request['amount'] ) {
			return new WP_Error( 'woocommerce_rest_invalid_order_refund', __( 'Refund amount must be greater than zero.', 'woocommerce-point-of-sale' ), 400 );
		}
		...
		if ( ! $refund ) {
			return new WP_Error( 'woocommerce_rest_cannot_create_order_refund', __( 'Cannot create order refund, please try again.', 'woocommerce-point-of-sale' ), 500 );
		}
