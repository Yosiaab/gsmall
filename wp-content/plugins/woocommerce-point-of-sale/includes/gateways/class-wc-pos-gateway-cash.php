...
class WC_POS_Gateway_Cash extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			...
			$order->update_status( apply_filters( 'woocommerce_pos_cash_process_payment_order_status', $order->has_downloadable_item() ? 'on-hold' : 'processing', $order ), __( 'Payment to be made upon delivery.', 'woocommerce-point-of-sale' ) );
		} else {
